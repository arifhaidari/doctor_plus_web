...
from appointment.models import Appointment, DaySchedulePattern, Feedback, ApptConditionTreat, ApptQrCode, FeedbackReplies
...
def save_clinic(clinic_dict, doctor, request):
    ...
    # when registered_clinics is empty it shows error then I should handle it 
        ...
        # input_length = 0
        clinic_dict_formated = dict(clinic_dict)
        registered_clinic_id_list = list(clinic_dict.getlist('registered_clinics', None)) 
        doctor_created_id_list = list(clinic_dict.getlist('clinic_id_list', None)) 
        input_list_no = len(list(clinic_dict.getlist("clinic_name", None)))
        
        print('value fo input_list_no')
        print(input_list_no)
        
        previous_clinics = Clinic.objects.filter(doctor=doctor)
        
        combined_clinic_id_list = registered_clinic_id_list + doctor_created_id_list
        print('value of len(registered_clinic_id_list) + input_list_no)')
        print(len(registered_clinic_id_list) + input_list_no)
        if (len(registered_clinic_id_list) + input_list_no) > 4:
            print('doctor is not allowed to have more than four clinics')
            messages.error(request, 'You have more than four clinics in your practice address list. Doctors only allowed to have four or less clinics')
            return
        
        print('value fo combined_clinic_id_list')
        print(combined_clinic_id_list)
        if previous_clinics.exists():
            # Remove Clinic
            for clinic_object in previous_clinics:
                if str(clinic_object.id) not in combined_clinic_id_list:
                    print('remvoe clinic', clinic_object)
                    clinic_object.doctor.remove(doctor)
                    # delete all appointment except completed 
                    doctor.appointment_set.filter(Q(~Q(status=Appointment.ApptStatus.COMPLETED) & Q(clinic=clinic_object))).delete()
                    # remvoe schedule pattern as well
                    DaySchedulePattern.objects.filter(clinic=clinic_object, doctor=doctor).delete()
                    if clinic_object.created_by == doctor:
                        print('clinic is also created by this doctor')
                        # if there is no completed appt or no doctor associated with it then we delete the clinic
                        is_appt_associated = Appointment.objects.filter(clinic=clinic_object, status=Appointment.ApptStatus.COMPLETED)
                        if clinic_object.doctor.count() == 0 and len(is_appt_associated) == 0:
                            clinic_object.delete()
            # Add Clinic
            existing_clinic_id_list = Clinic.objects.filter(doctor=doctor).values_list('id', flat=True)
            print('value fo existing_clinic_id_list')
            print(existing_clinic_id_list)
            # add the registered clinics
            for clinic_id in registered_clinic_id_list:
                if int(clinic_id) not in existing_clinic_id_list:
                    print('add clinic ', clinic_id)
                    new_clinic_obj = Clinic.objects.get(id=clinic_id)
                    new_clinic_obj.doctor.add(doctor)
        
        if input_list_no != 0:
            for index in range(input_list_no):
                print('value of index in main method', index)
                is_clinic = Clinic.objects.filter(
                    Q(clinic_name__iexact=clinic_dict.getlist("clinic_name")[index])
                    | Q(rtl_clinic_name=clinic_dict.getlist("rtl_clinic_name")[index])
                )
                if is_clinic.exists():
                    print('lets see which condition is true')
                 
                    print(is_clinic.exists())
                    try:
                        if clinic_dict.getlist('clinic_id_list')[index]:
                            # update existing clinic
                            the_clinic = is_clinic.first()
                            create_or_update_clinic(clinic_dict, doctor, index, the_clinic) 
                    except IndexError:
                        same_clinic = is_clinic.first()
                        messages.error(
                        request,
                        "{} is already existed. Please select the clinic instead of adding a new one or if it is same add an alias to make it different".format(
                            same_clinic.clinic_name or same_clinic.rtl_clinic_name
                        ))
                else:
                    # create new clinic
                    print('isndie he else ')
                    try:
                        if clinic_dict.getlist('clinic_id_list')[index]:
                            print('----==== what condti is true .... ')
                            the_clinic_obj = Clinic.objects.get(id=clinic_dict.getlist('clinic_id_list')[index])
                            create_or_update_clinic(clinic_dict, doctor, index, the_clinic_obj) 
                        else:
                            print('wht else is true 000000-----')
                    except IndexError:
                        print('create new clinic part one')
                        create_or_update_clinic(clinic_dict, doctor, index) 
                    
 
        else:
            print('create new clinic part two')
            if input_list_no != 0:
                for index in range(input_list_no):
                    create_or_update_clinic(clinic_dict, doctor, index)
...
def create_or_update_clinic(clinic_dict, doctor, index, clinic_object = None):
    print("inside the create_new_clinic")
    print('value of index in save clinic', index)
    try:
        city_object = City.objects.get(id=clinic_dict.getlist("clinic_city")[index])
        district_object = District.objects.get(id=clinic_dict.getlist("clinic_district")[index])
        if clinic_object is not None:
            clinic_object.clinic_name = clinic_dict.getlist("clinic_name")[index]
            clinic_object.rtl_clinic_name = clinic_dict.getlist("rtl_clinic_name")[index]
            clinic_object.address = clinic_dict.getlist("clinic_address")[index]
            clinic_object.rtl_address = clinic_dict.getlist("rtl_clinic_address")[index]
            clinic_object.city = city_object
            clinic_object.district = district_object
            clinic_object.save()
        else:
            new_clinic_object = Clinic.objects.create(
                clinic_name=clinic_dict.getlist("clinic_name")[index],
                rtl_clinic_name=clinic_dict.getlist("rtl_clinic_name")[index],
                address=clinic_dict.getlist("clinic_address")[index],
                rtl_address=clinic_dict.getlist("rtl_clinic_address")[index],
                city=city_object,
...
class DoctorProfile(View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            self.data.update({"doctor_object": request.user.doctor})
            the_data = self.data
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            if raw_data.get('operation') == 'save_changes' or raw_data.get('operation') == 'submit_for_review':
                ...
                save_clinic(raw_data, doctor, self.request)
